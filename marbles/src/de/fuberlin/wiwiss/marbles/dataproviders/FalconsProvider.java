...
public class FalconsProvider implements DataProvider {
	
	/**
	 * The URL of Falcon's lookup service 
	 */
	private final String SERVICE_URL = "http://iws.seu.edu.cn/services/falcons/api/whereis.jsp?uri=";
	 ...
	 */
	private String getFalconsLookupURL(Resource resource) {
		try {
	...
	public org.apache.commons.httpclient.URI getQueryURL(Resource resource) {
		org.apache.commons.httpclient.URI queryURL = null;
			try {
				String falconsLookupURL = getFalconsLookupURL(resource);
				if (falconsLookupURL != null)
					queryURL =  new org.apache.commons.httpclient.URI(falconsLookupURL, true /* is now escaped!! */);
	...
	public List<URI> getURLsFromData(CacheController cacheController, RepositoryConnection conn, Resource resource) {
		...
		if (null == (lookupURL = getFalconsLookupURL(resource)))
			return null;
		...
		try {
			statements = conn.getStatements(null, RDFS.SEEALSO, null, true, lookupURLContext);
