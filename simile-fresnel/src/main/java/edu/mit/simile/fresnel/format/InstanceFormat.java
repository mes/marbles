...
/**
 * Format that applies to a specific rdfs:Resource.
 * 
 * @author ryanlee
 */
public class InstanceFormat extends Format {
	/**
	...
	/**
	 * Constructor based on resource identifier
	 * 
	 * @param id A <code>Resource</code>
	 */
	public InstanceFormat(Resource id) {
		super(id);
	}
	...
	throws UnresolvableException, ParsingException {
		InstanceFormat out = new InstanceFormat(selected);

		try {
			// Parse domain
			InstanceFormat.parseDomain(out, in, selected, conf);
